...
begin
  ...
  insert into public.team_memberships (team_id, player_id, role, status, joined_at)
  values (target_invite.team_id, actor_player_id, 'member', 'active', now())
  on conflict (team_id, player_id) do update
  set status = 'active', joined_at = now();

  update public.team_invites
     set status = 'accepted', player_id = actor_player_id
   where id = target_invite.id;

  select * into accepted_team
    from public.teams
   where id = target_invite.team_id;
