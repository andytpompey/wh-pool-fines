...
alter table teams add column if not exists rackem_import_enabled boolean not null default false;
alter table teams add column if not exists rackem_league_slug text;
alter table teams add column if not exists rackem_league_name text;
alter table teams add column if not exists rackem_team_id text;
alter table teams add column if not exists rackem_team_name text;
alter table teams add column if not exists rackem_team_url text;
alter table seasons add column if not exists source text;
alter table seasons add column if not exists source_league_slug text;
alter table seasons add column if not exists source_season_team_id text;
alter table seasons add column if not exists source_url text;
alter table seasons add column if not exists source_last_refreshed_at timestamptz;
alter table seasons add column if not exists source_last_refresh_status text;
...
alter table matches add column if not exists source_home_team_id text;
alter table matches add column if not exists source_away_team_id text;
alter table matches add column if not exists source_team_role text;
alter table matches add column if not exists source_venue_id text;
alter table matches add column if not exists source_venue_name text;
alter table matches add column if not exists source_team_venue_id text;
alter table matches add column if not exists source_team_venue_name text;
alter table matches add column if not exists source_last_seen_at timestamptz;
create unique index if not exists seasons_rackem_source_unique_idx on seasons(team_id, source, source_season_team_id) where source is not null and source_season_team_id is not null;
create unique index if not exists matches_rackem_source_unique_idx on matches(team_id, source, source_identity) where source is not null and source_identity is not null;
...
create index if not exists audit_logs_team_created_at_idx on audit_logs(team_id, created_at desc);
create index if not exists audit_logs_action_idx on audit_logs(action);
create index if not exists audit_logs_actor_user_idx on audit_logs(actor_user_id);

alter table audit_logs enable row level security;
create policy "audit logs insert for members" on audit_logs for insert with check (
  exists (
    select 1
    from team_memberships tm
  ...
  or exists (
    select 1 from app_users au where au.id = auth.uid() and au.is_platform_admin = true
  )
);
create policy "audit logs select for platform admins" on audit_logs for select using (
  exists (
    select 1 from app_users au where au.id = auth.uid() and au.is_platform_admin = true
  )
);
