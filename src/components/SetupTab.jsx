      ...
      {section === 'players' && (
        <div>
          <div className="flex gap-2 mb-4">
            <input value={playerInput} onChange={e => setPlayerInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && addPlayer()} placeholder="Player name..."
            ...
            {[...players].sort((a, b) => a.name.localeCompare(b.name)).map(p => (
              <div key={p.id} className="flex items-center justify-between bg-zinc-800 rounded-lg px-3 py-2">
                ...
                <div className="flex items-center gap-2">
                  <button onClick={() => setEditPlayer({ id: p.id, name: p.name })} className="text-xs px-2 py-1 rounded-lg bg-zinc-700 hover:bg-zinc-600 text-zinc-300 font-bold">Edit</button>
                  <button onClick={() => { setConfirmDeletePlayer(p); setPlayerPinInput(''); setPlayerPinError('') }} className="text-red-400 hover:text-red-300 text-xl leading-none">×</button>
                </div>
              </div>
            ))}
          ...
          {editPlayer && (
            <Modal title="Edit Player" onClose={() => setEditPlayer(null)}>
              <Input label="Player Name" value={editPlayer.name} onChange={e => setEditPlayer(p => ({ ...p, name: e.target.value }))} onKeyDown={e => e.key === 'Enter' && saveEditPlayer()} />
              <div className="flex gap-2 mt-1"><Btn onClick={saveEditPlayer} className="flex-1">Save</Btn><Btn variant="ghost" onClick={() => setEditPlayer(null)} className="flex-1">Cancel</Btn></div>
            </Modal>
          )}
        </div>
      )}
