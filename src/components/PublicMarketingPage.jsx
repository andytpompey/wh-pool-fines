...
function Status(){const[components,setComponents]=useState([]);const[incidents,setIncidents]=useState([]);useEffect(()=>{Promise.all([supabase.from('service_components').select('*').order('name'),supabase.from('service_incidents').select('*').order('started_at',{ascending:false}).limit(20)]).then(([a,b])=>{setComponents(a.data??[]);setIncidents(b.data??[])})},[]);return <><Hero eyebrow="Service status" title="RooBin availability" body="Current component state and recent public incidents."/><div className="mx-auto max-w-3xl space-y-3 px-4 pb-16">{components.map(c=><div key={c.code} className="flex justify-between rounded-xl border border-zinc-800 bg-zinc-900 p-4"><span>{c.name}</span><span className={c.status==='operational'?'text-emerald-400':'text-amber-400'}>{c.status.replaceAll('_',' ')}</span></div>)}<h2 className="pt-6 text-xl font-bold">Recent incidents</h2>{incidents.length?incidents.map(i=><article key={i.id} className="rounded-xl border border-zinc-800 bg-zinc-900 p-4"><div className="flex justify-between"><strong>{i.title}</strong><span>{i.status}</span></div><p className="mt-2 text-zinc-400">{i.public_message}</p></article>):<p className="text-zinc-400">No public incidents recorded.</p>}</div></>}
