...
export default function TeamSubscriptionPanel({ team, seasons, canManageTeam }) {
  ...
  useEffect(() => {
    ...
    Promise.all([commercial.getPublishedTeamSeasonOffer(), commercial.getTeamPlayingCycles(team.id), commercial.getTeamBillingContext(team.id), commercial.getPendingBillingTransfers()]).then(async ([nextOffer, nextCycles, nextBilling, nextTransfers]) => {
      ...
      setBillingProfile({ name: nextBilling?.billingName ?? '', email: nextBilling?.billingEmail ?? '', line1: nextBilling?.billingAddress?.line1 ?? '', line2: nextBilling?.billingAddress?.line2 ?? '', city: nextBilling?.billingAddress?.city ?? '', postcode: nextBilling?.billingAddress?.postcode ?? '', countryCode: nextBilling?.billingAddress?.countryCode ?? 'GB', taxIdentifier: nextBilling?.taxIdentifier ?? '', reason: '' })
  ...
  return (
    <section className="space-y-4">
      <div className="rounded-2xl border border-zinc-800 bg-zinc-900 p-4">
        ...
        {canManageTeam && billingReview && <div className="mt-3 rounded-xl border border-zinc-700 bg-zinc-950 p-3 text-xs text-zinc-300"><p className="font-bold text-white">Before opening secure billing</p><p className="mt-1">The standard GBP 10 team-season purchase does not renew automatically. A captain purchases on behalf of the team. Cancellation prevents any future purchase but access continues until the displayed expiry date. Part-used periods are not normally refunded except where required by law, and expired access remains read-only.</p><div className="mt-2 flex gap-2"><Btn variant="outline" size="sm" onClick={async () => {
            ...
            return (
              <div key={item.cycle.id} className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-zinc-800 bg-zinc-900 p-4">
                <div>
                  ...
                  {item.subscriptionState === 'past_due' && <p className="mt-1 text-xs font-semibold text-amber-300">Payment failed — billing owner action is required during the grace period.</p>}
                  {item.state === 'missing' && <p className="mt-1 text-xs text-zinc-500">Awaiting team purchase or activation through an approved league grant.</p>}
      ...
      {billing?.isOwner && <div className="rounded-xl border border-zinc-800 bg-zinc-900 p-4"><h4 className="font-bold text-white">Billing profile</h4><p className="mt-1 text-xs text-zinc-400">Sensitive changes require a sign-in from the last 10 minutes and are audited.</p><div className="mt-2 grid grid-cols-2 gap-2">{[['name','Billing name'],['email','Billing email'],['line1','Address line 1'],['line2','Address line 2'],['city','Town or city'],['postcode','Postcode'],['countryCode','Country code'],['taxIdentifier','Tax identifier (optional)']].map(([key,label]) => <label key={key} className="text-xs text-zinc-400">{label}<input className="mt-1 w-full rounded-lg border border-zinc-700 bg-zinc-950 p-2 text-white" value={billingProfile[key]} onChange={event => setBillingProfile(current => ({ ...current, [key]: event.target.value }))}/></label>)}</div><label className="mt-2 block text-xs text-zinc-400">Change reason<input className="mt-1 w-full rounded-lg border border-zinc-700 bg-zinc-950 p-2 text-white" value={billingProfile.reason} onChange={event => setBillingProfile(current => ({ ...current, reason: event.target.value }))}/></label><Btn size="sm" className="mt-3" disabled={!billingProfile.name || !billingProfile.email || !billingProfile.line1 || !billingProfile.city || !billingProfile.postcode || billingProfile.reason.trim().length < 8} onClick={async () => { try { await commercial.updateBillingCustomerProfile(billing.id,billingProfile); setBilling(await commercial.getTeamBillingContext(team.id)); setStatus(current => ({ ...current, error: '' })) } catch (error) { setStatus(current => ({ ...current, error: error?.message ?? 'Billing profile could not be updated.' })) } }}>Save billing profile</Btn></div>}
      {billing?.isOwner && <div className="rounded-xl border border-zinc-800 bg-zinc-900 p-4"><h4 className="font-bold text-white">Billing contacts</h4><p className="mt-1 text-xs text-zinc-400">Grant or remove billing-only access for an existing verified RooBin account. Team roles are unaffected.</p><label className="mt-2 block text-xs text-zinc-400">Contact email<input type="email" className="mt-1 w-full rounded-lg border border-zinc-700 bg-zinc-950 p-2 text-white" value={billingContact.email} onChange={event => setBillingContact(current => ({ ...current,email:event.target.value }))}/></label><div className="mt-2 grid grid-cols-2 gap-2"><label className="text-xs text-zinc-400">Role<select className="mt-1 w-full rounded-lg border border-zinc-700 bg-zinc-950 p-2 text-white" value={billingContact.role} onChange={event => setBillingContact(current => ({ ...current,role:event.target.value }))}><option value="viewer">Viewer</option><option value="administrator">Administrator</option></select></label><label className="text-xs text-zinc-400">Action<select className="mt-1 w-full rounded-lg border border-zinc-700 bg-zinc-950 p-2 text-white" value={billingContact.action} onChange={event => setBillingContact(current => ({ ...current,action:event.target.value }))}><option value="grant">Grant</option><option value="remove">Remove</option></select></label></div><label className="mt-2 block text-xs text-zinc-400">Reason<input className="mt-1 w-full rounded-lg border border-zinc-700 bg-zinc-950 p-2 text-white" value={billingContact.reason} onChange={event => setBillingContact(current => ({ ...current,reason:event.target.value }))}/></label><Btn size="sm" variant="outline" className="mt-3" disabled={!billingContact.email || billingContact.reason.trim().length < 8} onClick={async () => { try { await commercial.manageBillingCustomerContact(billing.id,billingContact); setBilling(await commercial.getTeamBillingContext(team.id)); setBillingContact({ email:'',role:'viewer',action:'grant',reason:'' }); setStatus(current => ({ ...current,error:'' })) } catch (error) { setStatus(current => ({ ...current,error:error?.message ?? 'Billing contact could not be changed.' })) } }}>{billingContact.action === 'grant' ? 'Grant contact access' : 'Remove contact access'}</Btn></div>}
      {editing && (
